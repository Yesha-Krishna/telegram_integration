...
TOKEN = "7177299800:AAHVyt9iDJGukGdHaVKXJojKStEgQyxG0tI"
WEBHOOK_URL = "https://b444-2401-4900-1ce2-e717-89f3-1852-1662-fc4.ngrok-free.app/api/method/telegram_productivity.telegram_productivity.utils.telegram_bot.webhook"
...
#     connection_pool_size=20,  # Adjust this value based on your needs
#     pool_timeout=10  # Adjust this value based on your needs
# )
...
# 		expense_claim_doc.docstatus = 0
# 		expense_claim_doc.workflow_state = status
# 	elif status == "HR Approved":
# 		expense_claim_doc.docstatus = 1
# 		expense_claim_doc.workflow_state = status
# 	elif status == "HR Rejected":
# 		expense_claim_doc.docstatus = 1
# 		expense_claim_doc.workflow_state = status
...
# 	print("Entered start")
# 	logger.info(f"Handled /start command for chat_id: {update.effective_chat.id}")
# 	await bot.send_message(chat_id=update.effective_chat.id, text="Hello, I'm a bot. Please use /register to register.")
...
# 		message = f"A new expense claim has been approved by {from_user} . \nDo you want to approve or reject?"
# 		callback_data_approve = json.dumps({"status":"HR Approved","docname":doc_name})
# 		callback_data_reject = json.dumps({"status":"HR Rejected","docname":doc_name})
# 		keyboard = [
# 			[InlineKeyboardButton("Approve", callback_data= callback_data_approve)],
# 			[InlineKeyboardButton("Reject", callback_data=callback_data_reject)]
# 		]
...
# 	await context.bot.send_message(chat_id=update.effective_chat.id, text="Enter your email address")
...
# 		text_message = ''
# 		if 'status' in callback_data.keys():
...
# 			elif callback_data.get('status') == 'HR Approved':
# 				if update_doc_status(docname=callback_data.get('docname'),status=callback_data.get('status')):
# 					text_message = "Expense claim has been approved"
# 				else:
# 					print("error occured on update_doc_status")
# 			elif callback_data.get('status') == 'HR Rejected':
# 				if update_doc_status(docname=callback_data.get('docname'),status=callback_data.get('status')):
# 					text_message = "Expense claim has been rejected"
...
# 		if 'msg' in callback_data.keys() and 'action' in callback_data.keys():
...
# 	try:
# 		frappe.log_error("Telegram Webhook query", f"Context object: {context}")
# 		query = update.callback_query
# 		data = query.data
# 		message = ''
# 		if data == 'Approve':
...
# 		# # Read the incoming request data from Telegram 
...
# 		# Process the update using application
# 		update = Update.de_json(json.loads(request.data), bot)
# 		print(f"request: {update}")
# 		user_message = update.message.text
...
# 		return "Empty request data", 400
# 	except json.JSONDecodeError as je:
# 		logger.error(f"JSONDecodeError: Failed to decode JSON object: {je}")
...
# 		return "500"
	...
	# update_json = frappe.request.get_json()
...
# 	'https://alfarsi.aerele.in/api/method/telegram_productivity.telegram_productivity.utils.telegram_bot.webhook'
# 	success = bot.set_webhook(webhook_url)
# 	logger.info(f"Webhook set with URL: {webhook_url}, success: {success}")
# 	return success
...
# def start():
# 	print("Starting bot...")
# 	application = ApplicationBuilder().token(token=TOKEN).pool_timeout(30).build()
...
# if __name__ == '__main__':
	...
	# import tracemalloc
	# tracemalloc.start()
	# logger.info("Tracemalloc enabled for debugging memory issues.")
    
	# telegram_bot/telegram_bot/telegram_bot_handler.py
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    if context.args:
        ...
        print(args_str)
        await update.message.reply_text('Hi! Welcome to the Frappe Telegram Bot.')
    else:
        await update.message.reply_text('Hi! Welcome to the Frappe Telegram Bot.')
...
async def register(update: Update, context: CallbackContext) -> None:
    ...
    await update.message.reply_text('Here to help! You can register your Telegram account.')
